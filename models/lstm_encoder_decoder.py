...
class EncoderLSTM(nn.Module):
    def __init__(self,vocab_size,embed_dim,hidden_dim,no_layers=1,bidirectional=False,drop_out=0.2):
        ...
        Args:
            ...
            no_layers: number of layers required in LSTM
            bidirectional: if True will use bidirectional LSTM in encoder
            drop_out = drop out probability
        """
        super(EncoderLSTM, self).__init__()
      
        self.embedding = nn.Embedding(vocab_size,embed_dim)
        self.lstm = nn.LSTM(input_size=embed_dim,hidden_size=hidden_dim,num_layers=no_layers,batch_first=True,
    ...
    def forward(self,x):
        ...
        lstm_out, (hidden_state,cell_state) = self.lstm(embedding_out)  #[4,22,256] #[1,4,256] #[1,4,256]  #normal case
        ...
        if self.bidirectional:
            ...
            cell_state_backward, cell_state_forward = hidden_state[-1,:,:],hidden_state[-2,:,:]
            hidden_state = torch.concat((hidden_state_forward, hidden_state_backward),axis=1).unsqueeze(0)
            cell_state = torch.concat((cell_state_forward, cell_state_backward),axis=1).unsqueeze(0)
...
class DecoderLSTM(nn.Module):
    def __init__(self,vocab_size,embed_dim,hidden_dim,output_size,no_layers=1,drop_out=0.2):
    ...
    def forward(self, x, hidden_state, cell_state):
        ...
        lstm_out, (hidden_state,cell_state) = self.lstm(embedding_out,(hidden_state,cell_state)) #[4,1,256] #[1,4,256] #[1,4,256]
        out = hidden_state[-1:,:,:]  #taking the out from last layer 
...
class SeqtoSeqLSTM(nn.Module):
    def __init__(self, encoder, decoder,target_vocab_length,device):
        ...
        self.target_vocab_length = target_vocab_length
        self.device = device
    ...
    def forward(self, source, target,teacher_force_ratio=0.5):
        ...
        output = torch.zeros(target_len,batch_size,self.target_vocab_length).to(self.device)
        for t in range(1,target_len):
            out, hidden_state, cell_state = self.decoder(x, hidden_state, cell_state)   #[4,target_vocab_len]
